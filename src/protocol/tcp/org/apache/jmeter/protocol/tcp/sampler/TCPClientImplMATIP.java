...
/**
 * TCPClient implementation. Reads data until the defined EOM byte is reached.
 * If there is no EOM byte defined, then reads until the end of the stream is
 * reached. The EOM byte is defined by the property
 * "tcp.BinaryTCPClient.eomByte".
 *
 * Input data is assumed to be in hex, and is converted to binary
 */
public class TCPClientImplMATIP extends AbstractTCPClient {
    private static final Logger log = LoggerFactory.getLogger(TCPClientImplMATIP.class);

    private static final int EOM_INT = JMeterUtils.getPropDefault("tcp.BinaryTCPClient.eomByte", 1000); // $NON_NLS-1$

    public TCPClientImplMATIP() {
        super();
        setEolByte(EOM_INT);
        if (useEolByte) {
     ...
     * @return Byte array containing binary representation of input hex-encoded
     *         string
     * @throws IllegalArgumentException
     *             if string is not an even number of hex digits
     */
    ...
    /**
     * Reads data until the defined EOM byte is reached. If there is no EOM byte
     * defined, then reads until the end of the stream is reached. Response data
     * is converted to hex-encoded binary
     * 
        ...
        try {
            ...
            while ((x = is.read(buffer)) > -1) {
                if (first) {
                    ...
                    lenght = ((buffer[2] & 0xff) << 8) | (buffer[3] & 0xff);
                    first = false;
                }
                w.write(buffer, 0, x);
                if (useEolByte && (buffer[x - 1] == eolByte) || w.size() == lenght) {
                    break;
                }
